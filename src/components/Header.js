...
function Header() {
  const items = useSelector(selectItems); // useSelector is a hook
  const router = useRouter();  // useRouter is a hook
  const [session] = useSession();  // useSession is a hook
    ...
    <header className="sticky top-0 z-50">
      <div className="flex items-center flex-grow p-1 py-2 bg-amazon_blue">
        <div className="flex items-center flex-grow mt-2 sm:flex-grow-0">
          <Image
            onClick={() => router.push("/")} // onClick is a hook
